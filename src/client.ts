...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['RTCTEST_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['RTCTEST_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Rtctest API.
 */
export class Rtctest {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['RTCTEST_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('RTCTEST_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.RtctestError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Rtctest client with an apiKey option, like new Rtctest({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Rtctest.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('RTCTEST_LOG'), "process.env['RTCTEST_LOG']", this) ??
      defaultLogLevel;
  ...
  static RtctestError = Errors.RtctestError;
...
Rtctest.PetResource = PetResource;
Rtctest.Store = Store;
Rtctest.UserResource = UserResource;
export declare namespace Rtctest {
